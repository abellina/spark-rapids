...
class RapidsShuffleClientSuite extends RapidsShuffleTestHelper {
  def prepareBufferReceiveState(
    ...
    val tag = 123
    val ptr = PendingTransferRequest(client, tableMeta, tag, Seq.empty, mockHandler)
  ...
  def getShuffleRequests: Seq[ShuffleBlockBatchId] = {
    Seq(
      ShuffleBlockBatchId(0, 0, 0, 0),
      ShuffleBlockBatchId(0, 1, 1, 1),
      ShuffleBlockBatchId(0, 2, 3, 4))
  }
  ...
  test("successful metadata fetch") {
    ...
    val shuffleRequests = getShuffleRequests
    ...
    client.doFetch(shuffleRequests, mockHandler)
  ...
  test("errored metadata fetch") {
    when(mockTransaction.getStatus).thenReturn(TransactionStatus.Error)
    when(mockTransaction.getErrorMessage).thenReturn(Some("Error occurred"))

    val shuffleRequests = getShuffleRequests
    val contigBuffSize = 100000
    val tableMetas = RapidsShuffleTestHelper.mockMetaResponse(
  ...
  test("cancelled metadata fetch") {
    when(mockTransaction.getStatus).thenReturn(TransactionStatus.Cancelled)
    when(mockTransaction.getErrorMessage).thenReturn(Some("Cancelled"))
    ...
    // upon an errored response, the start andler will not be called
    verify(mockHandler, times(0)).start(any())
  ...
  test("successful buffer fetch") {
    ...
    withResource(bbs) { bounceBuffers =>
      ...
      verify(mockConnection, times(expectedReceives))
        .receive(any[Seq[AddressLengthTag]](), any())

      val total = mockConnection.receiveLengths.sum
      val count = mockConnection.receiveLengths.size

      assertResult(tableMeta.bufferMeta().actualSize())(total)
      assertResult(11)(count)

      // we would issue 1 request to issue a `TransferRequest` for the server to start
  ...
  test("cancelled buffer fetch") {
    ...
    // assume we obtained 2 bounce buffers, each 1/4 the size of the payload
    val numBuffers = 2
    val sizePerBuffer = numRows * 4/10
    val bbs = (0 until numBuffers).map( _ => DeviceMemoryBuffer.allocate(sizePerBuffer))
    withResource(bbs) { bounceBuffers =>
      ...
      // Cancelled transaction. Therefore we should not be done
      assert(!brs.isDone)
      ...
      verify(mockStorage, times(0)).addBuffer(any(), any(), any(), any())
