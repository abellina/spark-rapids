...
class WithPriorIterator[T <: AutoCloseable](prior: T, iter: Iterator[T])
  extends Iterator[T] {
  private[this] var priorConsumed: Boolean = false

  override def hasNext: Boolean = !priorConsumed || iter.hasNext
