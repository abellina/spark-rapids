    ...
    closeOnExcept(bufferedBuildIterator) { _ =>
      ...
      if (!firstBatchIsSerialized) {
        ...
        closeOnExcept(bufferedStreamIter) { _ =>
          val batcher = new MaybeHostConcatResultIterator(bufferedBuildIterator,
              batchSizeBytes, dataTypes, coalesceMetricsMap)
          closeOnExcept(batcher.next()) { maybeHostConcatResult =>
            // hasNext is false
            // then we have HostConcat result we need to do the hack with prior
            val buildBatch = if (!batcher.hasNext()) {
              ...
              // know we are now above `batchSizeBytes` (which is 2GB by default)
              val shuffleCoalesce = new GpuShuffleCoalesceIterator(
                new WithPriorIterator(maybeHostConcatResult, batcher),
