  ...
  override def onAllocFailure(allocSize: Long, retryCount: Int): Boolean = {
    ...
    try {
      withResource(new NvtxRange("onAllocFailure", NvtxColor.RED)) { _ =>
        val storedSize = store.currentSize
        val spillableSize = store.currentSpillableSize
        ...
        retryState.resetIfNeeded(retryCount, spillableSize)

        logInfo(s"Device allocation of $allocSize bytes failed, device store has " +
          s"$spillableSize spillable and $storedSize stored bytes. $attemptMsg" +
        ...
        if (spillableSize == 0) {
          ...
          val targetSize = Math.max(spillableSize - allocSize, 0)
