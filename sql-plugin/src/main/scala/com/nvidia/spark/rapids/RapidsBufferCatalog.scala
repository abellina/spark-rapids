...
class RapidsBufferCatalog(
    ...
    if (buffer.addReference()) {
      withResource(buffer) { _ =>
        ...
        if (!bufferHasSpilled) {
          ...
          val newBuffer = doSpill(buffer, spillStore, stream)
          // we get back a new RapidsBuffer instance in this new tier
          registerNewBuffer(newBuffer)
          // we can now remove the old tier linkage
          removeBufferTier(buffer.id, buffer.storageTier)
        } else {
          logWarning(s"Skipping spilling $buffer ${buffer.id} to ${spillStore.name} as it is " +
  ...
  /**
   * Spill `buffer` to `spillStore`, making room in `spillStore` if a maximum has been defined.
   */
  private def doSpill(
      buffer: RapidsBuffer,
      spillStore: RapidsBufferStore,
    ...
    var newBuffer: RapidsBuffer = null
    while (newBuffer == null) {
      newBuffer = spillStore.copyBuffer(buffer, buffer.getMemoryBuffer, stream)
      if (newBuffer == null) {
        // only the host store will return null above. We assume that the allocate function
