...
object RapidsConf {
  ...
  val SHUFFLE_MAX_IN_FLIGHT = 
    conf("spark.rapids.shuffle.maxInFlight")
      .doc("The amount max bytes in flight")
      .bytesConf(ByteUnit.BYTE)
      .createWithDefault(1024 * 1024 * 1024)
...
class RapidsConf(conf: Map[String, String]) extends Logging {
  ...
  lazy val shuffleMaxInFlight: Long = get(SHUFFLE_MAX_IN_FLIGHT)
