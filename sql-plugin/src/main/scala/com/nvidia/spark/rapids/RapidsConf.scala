...
object RapidsConf {
  ...
  val RMM_POOL = conf("spark.rapids.memory.gpu.pool")
    .doc("Select the RMM pooling allocator to use. Valid values are \"DEFAULT\", \"ARENA\", and " +
      ...
      "just passes through to CUDA memory allocation directly. Note: \"ARENA\" is currently " +
      "required if CUDF is built with Per-Thread Default Stream (PTDS), as \"DEFAULT\" is known " +
      "to be unstable (https://github.com/NVIDIA/spark-rapids/issues/1141)")
    .stringConf
