...
class RapidsShuffleClient(
  ...
  private[shuffle] def doIssueBufferReceives(bufferReceiveState: BufferReceiveState): Unit = {
    if (bufferReceiveState.firstTime) {
      ...
      logDebug(s"Not the first time around for " +
          s"${bufferReceiveState}, NOT sending transfer request.")
    }
    receiveBuffers(bufferReceiveState)
  }

  private def receiveBuffers(bufferReceiveState: BufferReceiveState): Transaction = {
    val sendBufferRange = new NvtxRange("receiveBuffs", NvtxColor.ORANGE)
    try {
      val buffersToReceive = bufferReceiveState.next()

      logInfo(s"Issuing receive for ${TransportUtils.formatTag(buffersToReceive.tag)} at " +
          s"state ${bufferReceiveState}")

      // TODO: stop using Seq version... just to make tests happy
      connection.receive(Seq(buffersToReceive),
        tx => {
          tx.getStatus match {
            case TransactionStatus.Success =>
              logInfo(s"Handling response for ${TransportUtils.formatTag(buffersToReceive.tag)}")
  ...
  private def queueTransferRequests(metaResponse: MetadataResponse,
    ...
    (0 until allTables).foreach { i =>
      ...
      if (tableMeta.bufferMeta() != null) {
        ...
        logDebug(s"Adding bufferId ${tableMeta.bufferMeta().id} -- " +
            s"tag ${TransportUtils.formatTag(tag)}, ${tableMeta.bufferMeta().size()} Bytes for " +
            s"handler ${handler}, ${this}")
