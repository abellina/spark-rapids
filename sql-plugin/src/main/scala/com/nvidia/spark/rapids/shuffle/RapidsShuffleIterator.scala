...
class RapidsShuffleIterator(
  ...
  /**
    ...
    * @param mapIndex - the map partition (as returned by the `MapOutputTracker` in
    *                 [[blocksByAddress]]
    ...
    (local ++ remote).foreach {
      case (blockManagerId: BlockManagerId, blockIds: Seq[(BlockId, Long, Int)]) => {
        ...
        val client = try {
          ...
          case t: Throwable => {
            logError(s"Error fetching ${blockIds} from ${blockManagerId}", t)
            shuffleRequestsMapIndex.map { case (req, mapIndex) =>
              resolvedBatches.offer(TransferError(
                blockManagerId, req, mapIndex, t.getMessage))
            }
            throw t
          }
        }
  ...
  override def next(): ColumnarBatch = {
    ...
    result match {
      ...
      case err @ TransferError(blockManagerId, shuffleBlockBatchId, mapIndex, errorMessage) =>
        ...
        logError(s"Transfer error detected by shuffle iterator. Failing task ${err}")
        throw new RapidsShuffleFetchFailedException(
          ...
          s"There was a fetch error from the rapids shuffle: ${errorMessage}")
    }
    cb
  }
}
