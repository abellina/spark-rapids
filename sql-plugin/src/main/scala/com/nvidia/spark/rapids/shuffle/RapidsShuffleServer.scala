    ...
    while (started) {
      closeOnExcept(new ArrayBuffer[BufferSendState]()) { bssToIssue =>
        ...
        while (!pendingTransfersQueue.isEmpty && continue) {
          ...
          val attempt = transport.tryGetSendBounceBuffers(rapidsConf.shuffleUcxBounceBuffersSize)
          if (attempt.nonEmpty) {
            attempt.foreach(_.close()) // free so we allocate what we actually need
      ...
      bufferSendStates.foreach { bufferSendState =>
        withResource(new NvtxRange(s"doHandleTransferRequest", NvtxColor.CYAN)) { _ =>
          ...
          withResource(buffersToSend) { _ =>
            ...
            // conversion after expanding the call in the method call below. So we have to define
            // the callback here in a val and type it to TransactionCallback
